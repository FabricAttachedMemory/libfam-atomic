...
void list_del(struct list *list, struct node *prev, struct node *node)
{
	if (!prev) {
...
int fam_atomic_register(void *address, int64_t length, int fd, int64_t offset)
{
...
void fam_atomic_unregister_region(void *address, int64_t length)
{
	...
	list_delete(fam_atomic_region_list, prev, curr);
}
...
static void fam_atomic_get_fd_offset(void *address, int *fd, int64_t *offset)
{
	...
	for (curr = fam_atomic_region_list->head; curr != NULL; curr = curr->next) {
		if (curr->region_start <= address &&
		    curr->region_start + curr->length >= address)
			break;
	}
...
int fam_atomic_32_compare_and_store_unpadded(int32_t *address,
...
int fam_atomic_64_compare_and_store_unpadded(int64_t *address,
...
{
	return fam_atomic_32_read_unpadded(&address->atomic_value);
}

int64_t fam_atomic_64_read(struct fam_atomic_64 *address)
{
	return fam_atomic_64_read_unpadded(&address->atomic_value);
}

void fam_atomic_32_write(struct fam_atomic_32 *address, int32_t value)
{
	fam_atomic_32_write_unpadded(&address->atomic_value, value);
}

void fam_atomic_64_write(struct fam_atomic_64 *address, int64_t value)
{
	fam_atomic_64_write_unpadded(&address->atomic_value, value);
}

int32_t fam_atomic_32_swap(struct fam_atomic_32 *address, int32_t value)
{
	return fam_atomic_32_swap_unpadded(&address->atomic_value, value);
}

int64_t fam_atomic_64_swap(struct fam_atomic_64 *address, int64_t value)
{
	return fam_atomic_64_swap_unpadded(&address->atomic_value, value);
}

int fam_atomic_32_compare_and_store(struct fam_atomic_32 *address,
				    int32_t expected, int32_t desired)
{
	return fam_atomic_32_compare_and_store_unpadded(&address->atomic_value,
							 expected, desired);
}

int fam_atomic_64_compare_and_store(struct fam_atomic_64 *address,
				    int64_t expected, int64_t desired)
{
	return fam_atomic_64_compare_and_store_unpadded(&address->atomic_value,
							 expected, desired);
}

int32_t fam_atomic_32_fetch_and_add(struct fam_atomic_32 *address, int32_t increment)
{
	return fam_atomic_32_fetch_and_add_unpadded(&address->atomic_value, increment);
}

int64_t fam_atomic_64_fetch_and_add(struct fam_atomic_64 *address, int64_t increment)
{
	return fam_atomic_64_fetch_and_add_unpadded(&address->atomic_value, increment);
}
